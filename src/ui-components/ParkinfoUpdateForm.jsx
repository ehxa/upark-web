...
import { getParkinfo } from "../graphql/queries";
import { updateParkinfo } from "../graphql/mutations";
const client = generateClient();
export default function ParkinfoUpdateForm(props) {
  const {
    id: idProp,
    parkinfo: parkinfoModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = parkinfoRecord
      ? { ...initialValues, ...parkinfoRecord }
      : initialValues;
    setFloors(cleanValues.floors);
    setNo_spots(cleanValues.no_spots);
    setCapacity(cleanValues.capacity);
    setDescription(cleanValues.description);
    setErrors({});
  };
  const [parkinfoRecord, setParkinfoRecord] = React.useState(parkinfoModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getParkinfo.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getParkinfo
        : parkinfoModelProp;
      setParkinfoRecord(record);
    };
    queryData();
  }, [idProp, parkinfoModelProp]);
  React.useEffect(resetStateValues, [parkinfoRecord]);
  const validations = {
    floors: [],
    no_spots: [],
    capacity: [],
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateParkinfo.replaceAll("__typename", ""),
            variables: {
              input: {
                id: parkinfoRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ParkinfoUpdateForm")}
      {...rest}
    >
      <TextField
        label="Floors"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || parkinfoModelProp)}
